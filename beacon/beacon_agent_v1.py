    ...
    def step(self, obs):
        ...
        if _MOVE_MARINE in obs.observation["available_actions"]:
            ...
            if self.choice == ACTION_DO_NOTHING:
                ...
                target = [marine_mean_x - 1, marine_mean_y - 1]

            elif self.choice == ACTION_U:
                target = [marine_mean_x, marine_mean_y - 1]

            elif self.choice == ACTION_UR:
                target = [marine_mean_x + 1, marine_mean_y - 1]

            elif self.choice == ACTION_L:
                target = [marine_mean_x - 1, marine_mean_y]
    ...
    try:
        while True:
            with sc2_env.SC2Env(
                ...
                data = [agent.beacon_count, str(datetime.datetime.now())]
